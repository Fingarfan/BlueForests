...
#Burial Activities
Burial <- matrix(c(0.3,0.7),
                 byrow = TRUE, ncol = 2,
                 dimnames = list(NULL, c("High Burial", "Low Burial")))

#Extreme Events
E_Events <- matrix(c(0.7875,1-0.7875),
                   byrow = TRUE, ncol = 2,
...
#Water Turbidity
Turbidity <- matrix(c(0.792647059, 1-0.792647059),
                    byrow = TRUE, ncol = 2,
                    dimnames = list(NULL, c("High Turbidity", "Low Turbidity")))

#Water Temperature
Temperature <- matrix(c(0.115625, 0.5125, 0.371875),
                      byrow = TRUE, ncol = 3,
                      dimnames = list(NULL, c("High WT", "Medium WT", "Low WT")))

#Grazers (conditional on Water Temperature)
Grazers <- matrix(c(0.8, 0.625, 0.45, 0.2, 0.375, 0.55),
                  byrow = TRUE, ncol = 3,
...
Alien_S <- matrix(c(0.803333333333333, 0.776666666666667, 0.776666666666667,  
                    0.196666666666667, 0.223333333333333, 0.223333333333333),
                  byrow = TRUE, ncol = 3,
...
#Bottom Trawling
Kelp.BT <- matrix(c(0.281818181818182, 0.327272727272727, 0.390909090909091,
                    0.345454545, 0.454545454545455, 0.2),
                  byrow = FALSE, ncol = 2,
                  dimnames = list(c("Good Kelp", "Average Kelp", "Low Kelp"), c("High BT","Low BT")))

#Burial Activities
Kelp.BA <- matrix(c(0.315384615384615, 0.423076923076923, 0.261538461538462, 
                    0.376923076923077, 0.423076923076923, 0.2),
                  byrow = FALSE, ncol = 2,
                  dimnames = list(c("Good Kelp", "Average Kelp", "Low Kelp"), c("High Burial","Low Burial")))

#Extreme Events
Kelp.EE <- matrix(c(0.2, 0.406666666666667, 0.393333333333333, 
                    0.44, 0.406666666666667, 0.153333333333333),
                  byrow = FALSE, ncol = 2,
                  dimnames = list(c("Good Kelp", "Average Kelp", "Low Kelp"), c("High Extreme","Low Extreme")))

#Alien Species
Kelp.AS <- matrix(c(0.35, 0.45, 0.2, 
                    0.45, 0.45, 0.1),
                  byrow = FALSE, ncol = 2,
                  dimnames = list(c("Good Kelp", "Average Kelp", "Low Kelp"), c("High Species","Low Species")))

#infectious Diseases
Kelp.ID <- matrix(c(0.283333333333333, 0.458333333333333, 0.258333333333333, 
                    0.425, 0.416666666666667, 0.158333333333333),
                  byrow = FALSE, ncol = 2,
                  dimnames = list(c("Good Kelp", "Average Kelp", "Low Kelp"), c("High Diseases","Low Diseases")))

#Grazers
Kelp.GRAZER <- matrix(c(0.05, 0.45, 0.5, 
                        0.3, 0.3, 0.4),
                      byrow = FALSE, ncol = 2,
                      dimnames = list(c("Good Kelp", "Average Kelp", "Low Kelp"), c("High Grazers","Low Grazers")))

#Water Turbidity
Kelp.TURB <- matrix(c(0.246153846153846, 0.553846153846154, 0.2, 
                      0.392307692307692, 0.507692307692308, 0.1),
                    byrow = FALSE, ncol = 2,
                    dimnames = list(c("Good Kelp", "Average Kelp", "Low Kelp"), c("High Turbidity","Low Turbidity")))

#Water Temperature
Kelp.TEMP <- matrix(c(0.123333333333333, 0.33, 0.546666666666667, 
                      0.246666666666667, 0.36, 0.393333333333333, 
                      0.346666666666667, 0.36,0.293333333333333),
                    byrow = FALSE, ncol = 3,
...
#Biodiversity
Biodiversity <- matrix(c(0.746153846153846, 0.153846154, 0.1, 
...
#Nursery Area
Nurs.Area <- matrix(c(0.6, 0.3, 0.1, 
                      0.35, 0.3, 0.35, 
                      0.2, 0.35, 0.45),
                    byrow = FALSE, ncol = 3,
...
C.Seq <- matrix(c(0.720833333333333, 0.2, 0.0791666666666667, 
                  0.375, 0.358333333333333, 0.266666666666667, 
                  0.1375, 0.316666666666667, 0.545833333333333),
                byrow = FALSE, ncol = 3,
                dimnames = list(c("High Sequestration", "Average Sequestration", "Low Sequestration"), c("Good Kelp", "Average Kelp", "Low Kelp")))

#Water Purification
Water.Pur <- matrix(c(0.75, 0.15, 0.1, 
                      0.5, 0.3, 0.2, 
                      0.15, 0.275, 0.575),
                    byrow = FALSE, ncol = 3,
...
#Aquaculture
Aquaculture <- matrix(c(0.284615384615385, 0.715384615384615, 
                        0.207692307692308, 0.792307692307692,
                        0.130769230769231, 0.869230769230769),
                      byrow = FALSE, ncol = 3,
                      dimnames = list(c("High Aquaculture", "Low Aquaculture"), c("Good Kelp", "Average Kelp", "Low Kelp")))

#Health Benefits
Health.Ben <- matrix(c(0.4, 0.6, 
                       0.3, 0.7,
                       0.2, 0.8),
                     byrow = FALSE, ncol = 3,
...
capture.output(print(cpt_kelp), file = "kelp_cpt_peniche.txt")
